...
Třída \InlCode{State} obsahuje tovární statickou metodu \InlCode{create}, která je další možností, jak vytvořit instanci stavu. Oproti vytvoření stavu pomocí konstruktoru nabízí pohodlnější cestu k vytvoření stavu, který obsahuje hlasový model (viz \ref{sec:VoiceModel}). Pomocí hlasového modelu definujeme typ stavu. Jedním z typů je stav, který zajišťuje přečtení vstupu od uživatele, dalším typem stavu může být stav, který slouží k prezentaci výstupu. Vytváření stavů pomocí teto metody je použito ve výpise č. \ref{src:MenuExample}.
...
Konstruktor třídy může být volán více způsoby. Ve výpise č. \ref{src:Prompt_Constructor} uvádím ukázky volání konstruktoru.
...
\paragraph{Třída Say}
\label{sec:Say}
Tato třída je odvozena ze třídy \InlCode{VoiceModel} a implementuje tedy konkrétní hlasový model. Prostřednictvím tohoto hlasového modelu sdělujeme uživateli nějakou zprávu. Způsoby vytvoření instance této třídy jsou ukázány ve výpise č. \ref{src:Say_Constructor}.
...
Ve výpise č. \ref{src:Say_Vxml} je ukázka VXML kódu, který pro takový stav aplikace automaticky vygeneruje.
...
Ve výpise č. \ref{src:Exit_Vxml} je uvedena ukázka VXML kódu, který je pro takový stav aplikací automaticky vygenerován.
...
Jelikož práce s tímto hlasovým modelem není demonstrována ve zdrojovým kódu ukázkové aplikace, tak ve výpise č. \ref{src:Record_example} uvádím ukázku zdrojového kódu, který vytváří stav, jehož hlasovým modelem je instance této třídy. V ukázce jsou demonstrovány dva způsoby zavolání konstruktoru.
...
Ve výpise č. \ref{src:Record_Vxml} je pro představu uveden VXML kód, který aplikace pro takový stav automaticky vygeneruje.
...
Pro stav, který má jako hlasový model uvedenu instanci třídy \InlCode{Ask}, aplikace automaticky vygeneruje VXML kód, který je předmětem výpisu č. \ref{src:Ask_Vxml}.
