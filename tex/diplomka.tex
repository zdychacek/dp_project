...
Na doprovodném CD jsou k dispozici zdrojové kódy webového a hlasového portálu včetně navrhované VXML knihovny.

\subsection{Diagramy jednotlivých částí hlasového portálu}
\label{sec:voice_portal_diagrams}
V této příloze se nachází diagramy všech částí hlasového portálu. Pro přehlednost zde uvádím odkazy na diagramy částí, které byly zmíněny přímo v textu práce.

\subsubsection{V textu uvedené diagramy}
\begin{itemize}
\item Hlavní struktura hlasového portálu je znázorněna na obrázku \ref{fig:MainFlow}.
\item Vnitřní stavy stavu $getLoginData$ je znázorněn na obrázku \ref{fig:GetLoginDataFlow}.
