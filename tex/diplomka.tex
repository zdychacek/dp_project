...
\CzechAbstract{Diplomová práce se zabývá návrhem a implementací webového a hlasového portálu na platformě VoiceXML.
...
\CzechKeywords{VoiceXML, VXML, hlasový portál, JSON, automatické rozpoznávání řeči, syntéza řeči, JavaScript, Node.js, AngularJS, Asterisk, VoiceGlue, Voxeo Prophecy, SIP}

\EnglishAbstract{Master’s thesis deals with design and implementation of web and voice portal with VoiceXML platform.

There is an introduction to related technologies like speech syntesis (TTS) and automatic speech recognition (ASR) covered too.
...
\EnglishKeywords{VoiceXML, VXML, voice portal, JSON, automatic speech recognition, speech synthesis, JavaScript, Node.js, AngularJS, Asterisk, VoiceGlue, Voxeo Prophecy, SIP}

% Pridame pouzivane zkratky (pokud nejake pouzivame).
\AddAcronym{XML}{Extensible Markup Language}
\AddAcronym{TCP/IP}{Transmission Control Protocol/Internet Protocol}
...
\AddAcronym{MRCP}{Media Resource Control Protocol}
\AddAcronym{CCXML}{Call Control eXtensible Markup Language}
\AddAcronym{SRGS}{Speech Recognition Grammar Specification}
\AddAcronym{SISR}{Semantic Interpretation for Speech Recognition}
...
\section{Úvod}
\label{sec:Uvod}
Tématem diplomové práce je návrh a implementace webového a hlasového portálu s podporou VoiceXML. Výběr tématu práce byl motivován mým zájmem navrhnout webový informační systém, který bude zároveň možné ovládat i pomocí samoobslužného hlasového kanálu a přinese tak uživatelům flexibilnější možnosti ovládání. V diplomové práci je popsán návrh webového a hlasového rozhraní informačního systému pro rezervaci letenek.

Druhá až čtvrtá kapitola se zabývají teoretickým úvodem do problematiky návrhu hlasových aplikací postavených na platformě VoiceXML.

V druhé kapitole je představen jazyk VoiceXML a jeho struktura. Kapitola pokračuje popisem vybraných hlasových platforem, na kterých je možné hlasové aplikace provozovat. Náplní závěrečné části kapitoly je seznámení s praktickým návrhem aplikací pomocí jazyka VoiceXML.

Třetí část představuje problematiku technologie TTS, tedy technologie syntézy umělé řeči.

Čtvrtá část se zabývá problematikou automatického rozpoznávání řeči pomocí technologie ASR.
...
\InsertFigure{Figures/vxml_architecture}{1\textwidth}{Architektura VoiceXML}{fig:voicexml_architecture}
...
\InsertFigure{Figures/voiceglue_architecture}{0.5\textwidth}{Architektura VoiceGlue}{fig:voiceglue_architecture}
...
\InsertFigure{Figures/vxml_document_structure}{0.8\textwidth}{Stromová struktura dokumentu VoiceXML}{fig:vxml_document_structure}
...
\lstinputlisting[language=XML,label=src:vxml_sample,caption={VoiceXML dokument}]{Vxml/vxml_sample.vxml}
...
\lstinputlisting[language=XML,label=src:vxml_menu,caption={Vytvoření menu pomocí elementu \texttt{<menu>}}]{Vxml/vxml_menu.vxml}
...
\lstinputlisting[language=XML,label=src:vxml_field,caption={Vytvoření formuláře pro získání uživatelské vstupu}]{Vxml/vxml_field.vxml}
...
\lstinputlisting[language=XML,label=src:vxml_grammar_sample,caption={Definice přípustných hodnot uživatelského vstupu pomocí gramatiky}]{Vxml/vxml_grammar_sample.vxml}
...
\InsertFigure{Figures/tts_scheme}{1\textwidth}{Proces převodu textu na řeč}{fig:tts_scheme}
...
\InsertFigure{Figures/asr_scheme}{1\textwidth}{Proces automatického rozpoznávání řeči}{fig:asr_scheme}
...
\InsertSidewaysFigure{Figures/web_flight_list}{1\textwidth}{Stránka zobrazující seznamem letů a filtr}{fig:FlightList}
...
\InsertSidewaysFigure{Figures/web_flight_detail}{1\textwidth}{Stránka zobrazující formulář pro editaci letu}{fig:FlightDetail}
...
\InsertSidewaysFigure{Figures/diagrams/mainflow}{1\textwidth}{Diagram popisující hlavní strukturu hlasového portálu}{fig:MainFlow}
...
\lstinputlisting[language=JavaScript,label=src:Prompt_Constructor,caption={Vytvoření instance třídy \texttt{Prompt}}]{Src/Prompt_Constructor.js}
...
\lstinputlisting[language=JavaScript,label=src:Say_Constructor,caption={Vytvoření instance třídy \texttt{Say}}]{Src/Say_Constructor.js}
...
\lstinputlisting[language=XML,label=src:Say_Vxml,caption={Kód vygenerovaný pro stav s hlasovým modelem \texttt{Say}}]{Vxml/Say.vxml}
...
\lstinputlisting[language=XML,label=src:Exit_Vxml,caption={Kód vygenerovaný pro stav s hlasovým modelem \texttt{Exit}}]{Vxml/Exit.vxml}
...
\lstinputlisting[language=XML,label=src:Record_Vxml,caption={Kód vygenerovaný pro stav s hlasovým modelem \texttt{Record}}]{Vxml/Record.vxml}
...
\lstinputlisting[language=XML,label=src:Ask_Vxml,caption={Kód vygenerovaný pro stav s hlasovým modelem \texttt{Ask}}]{Vxml/Ask.vxml}
...
\lstinputlisting[language=JavaScript,label=src:Var_example,caption={Použití zástupného objektu \texttt{Var}}]{Src/Var_example.js}
...
\lstinputlisting[language=JavaScript,label=src:Var_function,caption={Použití zástupného objektu \texttt{Var}}]{Src/Var_function.js}
...
\InsertFigure{Figures/diagrams/getdatedtmfflow}{0.8\textwidth}{Struktura komponenty pro zadání data}{fig:GetDateDtmfFlow}
...
\InsertFigure{Figures/diagrams/gettextinputdtmfflow}{0.35\textwidth}{Struktura komponenty pro zadání textového vstupu}{fig:GetTextInputDtmfFlow}
...
\InsertFigure{Figures/diagrams/getdestinationflow}{1\textwidth}{Struktura komponenty pro zvolení destinace}{fig:GetDestinationFlow}

\InsertFigure{Figures/diagrams/destinationselectionflow}{1\textwidth}{Vnitřní struktura stavu $destinationSelection$ komponenty pro zvolení destinace}{fig:DestinationSelectionFlow}
...
\InsertSidewaysFigure{Figures/Voxeo_admin}{0.9\textwidth}{Registrace nové VXML aplikace}{fig:Voxeo_admin}
...
\lstinputlisting[language=bash,label=src:Benchmark_bash_script,caption={Zdrojový kód skriptu \texttt{start-bench.sh}}]{Src/start-bench.sh}
...
V diplomové práci jsem se zabýval návrhem a následnou implementací webového a hlasového rozhraní informačního systému pro správu letenek. Hlasový portál byl navrhován na platformě VoiceXML.

V úvodní kapitole práce byl nejdříve představen jazyk VoiceXML a poté byly představeny vybrané hlasové platformy.

Další kapitoly se zabývaly popisem souvisejících technologií, konkrétně zde byla představena technologie pro syntézu řeči (TTS) a technologie pro automatické rozpoznávání řeči (ASR).

Hlavní náplň práce je obsažena v páté a šesté kapitole. Tyto kapitoly jsou věnovány popisu návrhu webového, resp. hlasového portálu. Byl zde detailně představen návrh knihovny, která má za úkol usnadnit návrh hlasových aplikací na platformě VoiceXML.

V poslední kapitole práce byly provedeny zátěžové testy navrženého hlasového portálu a byly tímto zjištěny reálné hardwarové a softwarové požadavky na provozní prostředí hlasového portálu.
...
\bibitem{speechlab} Technická univerzita v Liberci,
\textit{Laboratoř počítačového zpracování řeči}, https://www.ite.tul.cz/speechlab.
...
Na doprovodném CD jsou k dispozici zdrojové kódy webového a hlasového portálu včetně navržené VXML knihovny.

\subsection{Diagramy jednotlivých částí hlasového portálu}
\label{sec:voice_portal_diagrams}
...
\subsubsection{V textu uvedené diagramy}
\begin{itemize}
\item Hlavní struktura hlasového portálu je znázorněna na obrázku \ref{fig:MainFlow}.
\item Vnitřní struktura stavu $getLoginData$ je znázorněna na obrázku \ref{fig:GetLoginDataFlow}.
\item Struktura komponenty pro zadání data je znázorněna na obrázku \ref{fig:GetDateDtmfFlow}.
\item Struktura komponenty pro zadání textového vstupu je znázorněna na obrázku \ref{fig:GetTextInputDtmfFlow}.
\item Struktura komponenty pro zvolení destinace je znázorněna na obrázku \ref{fig:GetDestinationFlow}.
\item Vnitřní struktura stavu $destinationSelection$ komponenty pro zvolení destinace je znázorněna na obrázku \ref{fig:DestinationSelectionFlow}.
\end{itemize}

\subsubsection{V textu neuvedené diagramy}

\InsertSidewaysFigure{Figures/diagrams/appendix/createnewflow}{1\textwidth}{Struktura stavu $createNew <<CreateNewFlow>>$}{fig:createnewflow}

\InsertFigure{Figures/diagrams/appendix/cancelactiveflow}{1\textwidth}{Struktura stavu $cancelActive <<CancelActiveFlow>>$}{fig:cancelactiveflow}

\InsertFigure{Figures/diagrams/appendix/listactiveflow}{0.85\textwidth}{Struktura stavu $listActive <<ListActiveFlow>>$}{fig:listactiveflow}

\InsertFigure{Figures/diagrams/appendix/filterbyarrivaldateflow}{0.6\textwidth}{Struktura stavu $filterByArrivalDate <<FilterByArrivalDateFlow>>$}{fig:filterbyarrivaldateflow}

\InsertFigure{Figures/diagrams/appendix/filterbydeparturedateflow}{0.6\textwidth}{Struktura stavu $filterByDepartureDate <<FilterByDepartureDateFlow>>$}{fig:filterbydeparturedateflow}

\InsertFigure{Figures/diagrams/appendix/reservationmenuflow}{1\textwidth}{Struktura stavu $reservationMenu <<ReservationMenuFlow>>$}{fig:reservationmenuflow}

\InsertFigure{Figures/diagrams/appendix/reservationscontainerflow}{1\textwidth}{Struktura stavu $reservationsContainer <<ReservationsContainerFlow>>$}{fig:reservationscontainerflow}
