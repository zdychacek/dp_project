...
Na obrázku \ref{fig:GetTextInputDtmfFlow} je znázorněna struktura komponenty sloužící pro získání textového vstupu pomocí DTMF volby. Vstup se zadává stejným způsobem jako se na mobilním telefonu s klávesnicí píše SMS zpráva. Komponenta je implementována třídou \InlCode{GetTextInputDtmfFlow}.
...
Zpracování vstupu probíhá ve funkci, která je zaregistrovaná pro akci \InlCode{onExitAction} (viz \ref{sec:State}). Implementace této funkce spočívá v převedení vstupní číselné sekvence na textový formát. Převod se řídí mapováním, které je znázorněno v tabulce \ref{tab:GetTextInputDtmfFlow_mapping}. Např. následující sekvence 2446665 je převedena na řetězec 'ahoj'.
...
Stisk klávesy 0 je tedy nutný, pokud potřebujeme zadat dva po sobě jdoucí znaky, kdy každý z nich je namapován na stejnou klávesu. Pokud bychom např. potřebovali zadat textový řetězec 'abc', museli bychom zadat následující sekvenci 20220222.
...
\InsertFigure{Figures/diagrams/getdestinationflow}{1\textwidth}{Obecná struktura komponenty pro zvolení destinace (GetDestinationFlow)}{fig:GetDestinationFlow}

\InsertFigure{Figures/diagrams/destinationselectionflow}{1\textwidth}{Podrobnější struktura komponenty pro zvolení destinace (DestinationSelectionFlow)}{fig:DestinationSelectionFlow}
